...
class Text:
    ...
    def __init__(self, content: Any):
        ...
        self._suffix_codes: List[str] = []

    def style(self, color: Optional[str] = None, bg: Optional[str] = None, styles: Union[str, List[str], None] = None) -> 'Text':
    ...
    def add_raw_code(self, code: str) -> 'Text':
        """Injects a raw ANSI code string."""
        if code: self._codes.append(str(code))
        return self

    def add_raw_suffix(self, code: str) -> 'Text':
    ...
    def __str__(self) -> str:
        ...
        parts = []
        csi_params = [c for c in self._codes if c and c[0].isdigit()]
        if csi_params: parts.append(f"\033[{';'.join(csi_params)}m")

        raw_seqs = [c for c in self._codes if c and not c[0].isdigit()]
    ...
    def __repr__(self) -> str: return f"<Text: {self.content}>"
    def __add__(self, other: Any) -> str: return str(self) + str(other)
    def __radd__(self, other: Any) -> str: return str(other) + str(self)
